  ...
  ({ className, variant = 'default', size = 'default', children, loading, onClick, iconLeft, iconRight, ...props }, ref) => {
    ...
    return (
      <button
        ...
        {...props}
      >
        <span className="ui:relative ui:z-10 ui:flex ui:items-center ui:gap-2">
            {loading && <Loader2 className="ui:h-4 ui:w-4" />}
            {!loading && iconLeft && (
        ...
        {!loading && (
          <div className="ui:absolute ui:inset-0 ui:z-0">
            {ripples.map((ripple) => (
              <span
                key={ripple.id}
                className={cn("ui:absolute ui:rounded-full ui:animate-ripple", rippleColor)}
                style={{ 
                    left: ripple.x, 
                    top: ripple.y, 
                    width: ripple.size, 
