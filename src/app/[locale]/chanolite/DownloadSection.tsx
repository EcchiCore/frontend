...
import { 
    ...
    ArrowRight, ShieldAlert, Sparkles, HelpCircle, CheckCircle, ExternalLink, Feather, Gauge
} from 'lucide-react'
...
export default function DownloadSection({ releases }: { releases: Release[] }) {
    const t = useTranslations('ChanoLite')
    const tX2 = useTranslations('ChanoX2')
    ...
    // Screenshot Showcase Tab state (default: 'main')
    const [activeScreenshotTab, setActiveScreenshotTab] = useState<'main' | 'minimalist' | 'performance'>('main')
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center pt-4">
                ...
                <div className="lg:col-span-5 space-y-6 text-left">
                    <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-xl bg-zinc-900 border border-zinc-800 p-2.5 flex items-center justify-center shadow-sm select-none text-emerald-400">
                            <Feather className="w-full h-full object-contain" />
                        </div>
                        <span className="text-3xl font-extrabold tracking-tight text-zinc-50 font-mono">ChanoLite</span>
                    </div>

                    <p className="text-sm md:text-base text-zinc-400 leading-relaxed font-sans font-normal">
                        {t('heroDesc')} <span className="text-emerald-500 font-medium">{t('heroTagline')}</span>
                    </p>

                    {/* Massive installation action button */}
                    <div className="space-y-4 pt-2">
                        <Button size="lg" className="w-full sm:w-auto h-12 px-8 bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-bold rounded-lg shadow-[0_0_20px_-5px_rgba(16,185,129,0.3)] flex items-center justify-center gap-2 transition-colors text-sm shrink-0 border-none" asChild>
                            <a href={getDefaultInstallLink()}>
                                <FileDown className="w-5 h-5 shrink-0" />
                                <span>INSTALL CHANOLITE</span>
                            </a>
                        </Button>
                ...
                <div className="lg:col-span-7 space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="inline-flex rounded-lg bg-zinc-900/50 p-0.5 border border-zinc-800/80 text-[10px] font-medium shrink-0 font-sans">
                            <button
                                onClick={() => setActiveScreenshotTab('main')}
                                className={`px-2.5 py-1 rounded transition-all ${activeScreenshotTab === 'main' ? 'bg-zinc-800 text-emerald-400 shadow-sm font-semibold' : 'text-zinc-500 hover:text-zinc-300'}`}
                            >
                                Main UI
                            </button>
                            <button
                                onClick={() => setActiveScreenshotTab('minimalist')}
                                className={`px-2.5 py-1 rounded transition-all ${activeScreenshotTab === 'minimalist' ? 'bg-zinc-800 text-emerald-400 shadow-sm font-semibold' : 'text-zinc-500 hover:text-zinc-300'}`}
                            >
                                {t('featureStreamlined')}
                            </button>
                            <button
                                onClick={() => setActiveScreenshotTab('performance')}
                                className={`px-2.5 py-1 rounded transition-all ${activeScreenshotTab === 'performance' ? 'bg-zinc-800 text-emerald-400 shadow-sm font-semibold' : 'text-zinc-500 hover:text-zinc-300'}`}
                            >
                                {t('featureEfficiency')}
                            </button>
                        </div>
                    ...
                    {/* macOS-style Window Mockup containing ChanoLite real assets */}
                    <div className="rounded-xl border border-zinc-800 bg-zinc-950 overflow-hidden shadow-sm transition-all duration-300">
                        ...
                        <div className="h-9 bg-zinc-900/50 border-b border-zinc-800 px-4 flex items-center justify-between select-none">
                            ...
                            <div className="px-6 py-0.5 rounded bg-zinc-950 border border-zinc-900 text-[10px] text-zinc-500 font-mono">
                                {activeScreenshotTab === 'main' ? 'chanolite://main_dashboard' : activeScreenshotTab === 'minimalist' ? 'chanolite://minimalist_view' : 'chanolite://performance_dashboard'}
                            </div>
                            <div className="w-10"></div>
                        </div>
                        
                        {/* Mockup Body displaying beautiful screenshots */}
                        <div className="relative overflow-hidden bg-zinc-950 flex items-center justify-center aspect-[16/9]">
                            <img 
                                src={activeScreenshotTab === 'main' ? '/ChanoLite/20251220_103615.png' : activeScreenshotTab === 'minimalist' ? '/ChanoLite/20251220_103641.png' : '/ChanoLite/20251220_103644.png'} 
                                alt="ChanoLite Screen Preview"
            ...
            <div id="featured-downloads" className="space-y-6 pt-12 border-t border-zinc-900 scroll-mt-24">
                ...
                <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-${featuredCards.length} gap-6`}>
                    {featuredCards.map(card => (
                        <div 
                            key={card.id}
                            className={`relative group p-6 rounded-xl bg-zinc-900/30 border ${card.detected ? 'border-emerald-500/40 shadow-sm bg-zinc-900/50' : 'border-zinc-800/80'} hover:border-zinc-750 hover:bg-zinc-900/40 transition-all duration-200 flex flex-col justify-between`}
                        >
                            {card.detected && (
                                <span className="absolute top-3 right-3 bg-emerald-950 text-emerald-400 border border-emerald-900/50 text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded">
                                    Detected OS
                                </span>
                            )}
                                ...
                                >
                                    <div className="flex flex-wrap items-center gap-3">
                                        ...
                                        {release.prerelease ? (
                                            ...
                                            <Badge className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[10px] font-bold px-2 py-0.5 rounded font-sans">
                                                Stable
                                            </Badge>
                                        )}
